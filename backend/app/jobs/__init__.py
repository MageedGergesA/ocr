...
def start_batch(pdf_bytes: bytes, hard: bool, api_key_id: int, total_pages: int,
                owner_user_id: int) -> str:
    job_id = uuid.uuid4().hex
    with _LOCK:
        # `owner_user_id` is stored for authorization only. It is INTERNAL: the
        # /v1/jobs route strips it before returning so the poll response shape is
        # unchanged. Without it, possession of the (unguessable) job id was enough
        # to read another tenant's extracted document.
